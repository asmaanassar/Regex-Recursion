...
function capitalA(s) {
    ...
    if (str[str.length - 1] == 'A' && regex1.test(s) === false) {
        return true
    } else {
        return false
    }
...
function ioEmail(email) {
    ...
    let regex1 = /io\b/g
    let regex2 = /[.]/g
    let str = email.match(regex2) ;
    
  if(email.match(regex) && email.match(regex1)&& str.length <2){
    return true
  }else{
    return false
  }
...
function imagesSearcher(text) {
    let arr = [];
    let reg = /(\w)*.png|(\w)*.jpg|(\w)*.jpeg\b/gi
